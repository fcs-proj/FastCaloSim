...
void RTree::build(const std::string& index_path)
{
  if (!m_builder) {
  ...
  m_builder->build(index_path);

  // Release builder resources after building
  m_builder.reset();
}

void RTree::load(const std::string& index_path, size_t cache_size)
{
  if (!m_query) {
    m_query = std::make_unique<RTreeQuery>(m_coordinate_system);
  }

  m_query->load(index_path, cache_size);
}
