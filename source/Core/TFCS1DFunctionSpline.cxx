                                        ...
                                        int maxnp)
{
  ...
  for (int np = 3; np <= maxnp; ++np) {
    ...
    MSG_INFO("========== Spline #=" << np << " max_penalty_best="
                                    << max_penalty_best << " ==============");
    MSG(INFO) << "==== Best spline init | ";
                                                  ...
                                                  double maxeffsiggoal)
{
  ...
  int ind = 0;
  MSG(INFO) << "Spline init p_improve=" << p_improve << " | ";
  for (int i = 0; i < nsplinepoints; ++i) {
    double p, x;
    sp.GetKnot(i, p, x);
    if (i == 0 && p_improve < p) {
      nprop[ind] = (0 + p) / 2;
      MSG(INFO) << ind << " : pi=" << nprop[ind] << " ; ";
      ++ind;
    }

    nprop[ind] = p;
    MSG(INFO) << ind << " : p=" << nprop[ind] << " ; ";
    ++ind;
  ...
  MSG_INFO("Spline init spline #="
           << nsplinepoints << " : maxdev=" << maxdev
           << " p_maxdev=" << p_maxdev << " maxeffsig=" << maxeffsig
           << " p_maxeffsig=" << p_maxeffsig << " max_penalty=" << max_penalty);
  MSG(INFO) << "  ";
       ...
       dproploop /= 2)
  {
    ...
    for (int itry = 0; itry < ntry; ++itry) {
      ...
      if (max_penalty < 0 || penalty < max_penalty) {
        ...
        sp_best = sp;
        /*
        MSG(INFO) <<"#="<<nsplinepoints<<" try="<<itrytot-1<<" | ";
        for(int i=0;i<nsplinepoints;++i) {
          MSG(INFO) <<i<<":p="<<nprop_try[i]<<" x="<<nx[i]<<" ; ";
        }
        MSG(INFO) <<"new maxdev="<<maxdev<<" maxeffsig="<<maxeffsig<<"
                                        ...
                                        int ntoy)
{
  ...
  effN *= effN;
  // MSG_INFO("integral="<<integral<<" +- "<<interr<<"
  // relerr="<<interr/integral); MSG_INFO("effN="<<effN<<" +-
  ...
  for (int i = 0; i <= hist->GetNbinsX() + 1; i++) {
    ...
    // MSG_INFO(i<<": diff="<<int1-int2<<" sig(diff)="<<valsig<<"
