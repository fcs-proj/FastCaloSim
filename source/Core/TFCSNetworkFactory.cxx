...
void TFCSNetworkFactory::resolveGlobs(std::string& filename)
{
  ...
  if (filename_len < ending_len) {
    MSG_NOCLASS(logger, "Filename is implausably short.");
  } else if (0
             == filename.compare(filename_len - ending_len, ending_len, ending))
  {
    MSG_NOCLASS(logger, "Filename ends in glob.");
    // Remove the glob
    filename.pop_back();
...
std::unique_ptr<VNetworkBase> TFCSNetworkFactory::create(std::string input)
{
  ...
  if (VNetworkBase::isFile(input) && isOnnxFile(input)) {
    ...
    try {
      ...
      MSG_NOCLASS(logger, "Succeeded in creating LWTNN graph from string");
      return created;
    };
  };
};
                                                         ...
                                                         bool graph_form)
{
  ...
  if (VNetworkBase::isFile(input) && isOnnxFile(input)) {
    ...
    MSG_NOCLASS(logger, "Creating LWTNN graph from string");
    std::unique_ptr<VNetworkBase> created(new TFCSGANLWTNNHandler(input));
    return created;
  } else {
    std::unique_ptr<VNetworkBase> created(new TFCSSimpleLWTNNHandler(input));
    MSG_NOCLASS(logger, "Creating LWTNN nn from string");
    return created;
  };
};
...
{
  MLogging logger;
  MSG_NOCLASS(logger, "Given both bytes and a string to create an nn.");
