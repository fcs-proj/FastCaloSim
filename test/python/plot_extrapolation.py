        ...
        for layer_id in state_data.layer_id.unique():
            ...
            if layer_id == -1:
                ...
                if layer_id not in plotted_layers:
                    plt.plot(sub_pos_data.z, sub_pos_data.r, 'o', markersize=3, color='red', zorder=10, label = 'ID-Calo Boundary', linestyle='')
                    plotted_layers.add(layer_id)
                    
                else:
                    plt.plot(sub_pos_data.z, sub_pos_data.r, 'o', markersize=3, color='red', zorder=10, linestyle='')
                
                continue
                    
            # Get data for the current layer and requested subposition
            layer_data = state_data[(state_data.layer_id == layer_id) & (state_data.subpos == args.subpos)]
            ...
            if layer_id not in plotted_layers:
                plt.plot(layer_data.z, layer_data.r, color=colorList[layer_id % 10], marker=marker, markersize=3, label=atlas_calo_layers[layer_id].name, linestyle='')
                plotted_layers.add(layer_id)
            else:
                plt.plot(layer_data.z, layer_data.r, color=colorList[layer_id % 10], marker=marker, markersize=3, linestyle='')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--subpos', type=str, default="ENT", help='Subposition to plot (ENT, MID, EXT)')
