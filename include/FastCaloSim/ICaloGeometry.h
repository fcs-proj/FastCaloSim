...
#include "FastCaloSim/CaloDetDescrElement.h"
#include "FastCaloSim/FastCaloSim_CaloCell_ID.h"
#include "FastCaloSim/Identifier.h"
...
public:
  ...
  virtual const CaloDetDescrElement* getDDE(int sampling,
                                            float eta,
                                            float phi,
                                            float* distance = 0,
                                            int* steps = 0) = 0;
  ...
  virtual void minmaxeta(int sample,
                         double eta,
                         double& mineta,
  ...
  virtual double rpos(int sample,
                      double eta,
                      int subpos = CaloSubPos::SUBPOS_MID) const = 0;
  virtual double zpos(int sample,
                      double eta,
                      int subpos = CaloSubPos::SUBPOS_MID) const = 0;
  virtual double rzpos(int sample,
                       double eta,
                       int subpos = CaloSubPos::SUBPOS_MID) const = 0;
