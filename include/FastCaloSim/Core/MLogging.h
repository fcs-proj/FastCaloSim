...
#define MSG_LVL_NOCHK(enum_lvl, x) \
  do { \
    this->msg() << this->startMsg(enum_lvl, __FILE__, __LINE__) << x \
                << std::endl; \
  } while (0)

#define MSG_VERBOSE(x) MSG_LVL(MSG::VERBOSE, x)
#define MSG_DEBUG(x) MSG_LVL(MSG::DEBUG, x)
...
// Set up a stream that can be used like: MSG(INFO) << "hello" <<
// END_MSG(INFO); It needs to only write the left columns once, until it is
// fed another END_MSG

// Provide a stream
#define MSG(lvl) this->stream(MSG::lvl, __FILE__, __LINE__)
...
#define FCS_ENDMSG this->streamerEndLine(MSG::INFO)
